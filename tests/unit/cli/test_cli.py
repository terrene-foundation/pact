...
"""Tests for CARE Platform CLI entry point (Task 113).
...
class TestCLIVersion:
    ...
    def test_version_shows_care_platform(self, runner):
        from care_platform.build.cli import main

        result = runner.invoke(main, ["--version"])
        assert "care-platform" in result.output.lower() or "0.1.0" in result.output
...
class TestCLIModuleExecutable:
    def test_cli_module_importable(self):
        """Verify care_platform.build.cli is importable and has main."""
        from care_platform.build.cli import main
