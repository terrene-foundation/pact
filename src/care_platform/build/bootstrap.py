...
Provides the ``PlatformBootstrap`` class that takes a PlatformConfig (or YAML path)
...
Usage:
    config = PlatformConfig.model_validate(yaml.safe_load(config_yaml))
...
from care_platform.build.config.env import _load_dotenv
...
from care_platform.build.workspace.discovery import WorkspaceDiscovery
from care_platform.trust.store.sqlite_store import GenesisAlreadyExistsError
...
class PlatformBootstrap:
    """Initializes the CARE Platform trust hierarchy from configuration.

    Given a PlatformConfig and a TrustStore, creates the complete trust
    ...
    def load_config(config_path: str | Path) -> PlatformConfig:
        ...
        Returns:
            Parsed and validated PlatformConfig.
    ...
    def _create_genesis(self, config: PlatformConfig, result: BootstrapResult) -> None:
    ...
    def _create_envelopes(self, config: PlatformConfig, result: BootstrapResult) -> None:
    ...
    def _create_delegations(self, config: PlatformConfig, result: BootstrapResult) -> None:
