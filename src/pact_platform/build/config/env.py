...
Environment variables recognized by the PACT:

| Variable                    | Required | Default                                      | Description                        |
|-----------------------------|----------|----------------------------------------------|------------------------------------|
| PACT_API_TOKEN              | No*      | ""                                           | Bearer token for API auth          |
| PACT_API_HOST               | No       | 0.0.0.0                                      | API server bind host               |
| PACT_API_PORT               | No       | 8000                                         | API server bind port               |
| PACT_CORS_ORIGINS           | No       | http://localhost:3000,http://localhost:3001   | Comma-separated CORS origins       |
| PACT_MAX_WS_SUBSCRIBERS     | No       | 50                                           | Max WebSocket subscribers          |
| PACT_DEV_MODE               | No       | false                                        | Allow empty API token if true      |
| PACT_RATE_LIMIT_GET         | No       | 60/minute                                    | Rate limit for GET requests        |
| PACT_RATE_LIMIT_POST        | No       | 10/minute                                    | Rate limit for POST requests       |
...
| PACT_LOG_FORMAT             | No       | console                                      | Log output format (json or console)|

* PACT_API_TOKEN is required unless PACT_DEV_MODE=true.
"""
...
class EnvConfig:
    """Typed, validated environment configuration for the PACT."""

    # API
    pact_api_token: str = ""
    pact_api_host: str = "0.0.0.0"
    pact_api_port: int = 8000
    pact_cors_origins: list[str] = field(
        default_factory=lambda: ["http://localhost:3000", "http://localhost:3001"]
    )
    pact_max_ws_subscribers: int = 50
    pact_dev_mode: bool = False
    pact_rate_limit_get: str = "60/minute"
    pact_rate_limit_post: str = "10/minute"
...
def load_env_config(*, load_dotenv: bool = True) -> EnvConfig:
    ...
    cors_raw = os.environ.get("PACT_CORS_ORIGINS", "http://localhost:3000,http://localhost:3001")
    cors_origins = [o.strip() for o in cors_raw.split(",") if o.strip()]

    config = EnvConfig(
        pact_api_token=os.environ.get("PACT_API_TOKEN", ""),
        pact_api_host=os.environ.get("PACT_API_HOST", "0.0.0.0"),
        pact_api_port=_int_env("PACT_API_PORT", 8000),
        pact_cors_origins=cors_origins,
        pact_max_ws_subscribers=_int_env("PACT_MAX_WS_SUBSCRIBERS", 50),
        pact_dev_mode=_bool_env("PACT_DEV_MODE"),
        pact_rate_limit_get=_validate_rate_limit(
            os.environ.get("PACT_RATE_LIMIT_GET", "60/minute"), "PACT_RATE_LIMIT_GET"
        ),
        pact_rate_limit_post=_validate_rate_limit(
            os.environ.get("PACT_RATE_LIMIT_POST", "10/minute"), "PACT_RATE_LIMIT_POST"
        ),
        ...
        log_format=os.environ.get("PACT_LOG_FORMAT", "console"),
    )

    # Validate: PACT_API_TOKEN required in production mode
    if config.is_production and not config.pact_api_token:
        raise EnvConfigError(
            "PACT_API_TOKEN is required in production mode. "
            "Set PACT_API_TOKEN to a secure token, or set PACT_DEV_MODE=true "
            "for local development (not recommended for production)."
        )

    if config.pact_dev_mode and not config.pact_api_token:
