...
> **DataFlow v0.11.0 Update**: `auto_migrate=True` now works correctly in Docker/FastAPI environments using synchronous DDL internally (psycopg2/sqlite3 for synchronous DDL). The previous workaround of using `auto_migrate=False` + `create_tables_async()` is **OBSOLETE**.
>
...
# synchronous DDL handles table creation synchronously (no event loop issues)
```
...
- Works correctly in Docker/FastAPI with synchronous DDL support
