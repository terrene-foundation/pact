...
See `rules/zero-tolerance.md` for details.
...
Agents are organized by function. See `.claude/agents/` for the full set.

| Category           | Key Agents                                                                                |
| ------------------ | ----------------------------------------------------------------------------------------- |
| **Analysis**       | analyst (failure analysis + requirements)                                                 |
| **Frameworks**     | dataflow-specialist, nexus-specialist, kaizen-specialist, mcp-specialist, pact-specialist |
| **Quality**        | reviewer, security-reviewer, gold-standards-validator, testing-specialist                 |
| **Implementation** | tdd-implementer, pattern-expert, build-fix                                                |
| **Frontend**       | react-specialist, flutter-specialist, uiux-designer                                       |
| **Release**        | release-specialist, todo-manager, gh-manager                                              |
| **Project**        | pact-dashboard-specialist, pact-trust-specialist                                          |

Standards reference: use `co-reference` skill (covers CARE, EATP, CO, COC specs).

## Skills

SDK skills in `.claude/skills/01-*` through `17-*`. Standards in `co-reference/`, `26-eatp-reference/`, `29-pact/`. Project skills in `project/`.
...
# Async: await runtime.execute_workflow_async(workflow.build(), inputs={})
# Express CRUD: db.express_sync.create("Model", {...})  # 23x faster than workflows
# PactEngine: result = await pact_engine.submit(objective, role, context)
```
